import fs, { readFileSync, unlinkSync, writeFileSync, readdirSync, statSync } from 'fs';
...
hasha.fromFile = function async (filePath, options) { return hasha.fromStream(fs.createReadStream(filePath), options); };

hasha.fromFileSync = function (filePath, options) { return hasha(fs.readFileSync(filePath), options); };
...
function index (opt) {
	...
	return {
		...
		writeBundle: function writeBundle(config, data) {
			...
			var $ = cheerio.load(isHTML?template:readFileSync(template).toString());
			var head = $('head');
			var body = $('body');
			var entryConfig = {};
			...
			var destPath = relative('./', fileName);
			var destDir = dest || destPath.slice(0, destPath.indexOf(sep));
			var destFile = destDir + "/" + (filename || basename(template));
			...
			fileList.forEach(function (node) {
				...
				if (/\[hash\]/.test(file)) {
					...
					if (sourcemap) {
						...
						var srcmapCode = readFileSync(srcmapFile).toString();
				...
				var src = isURL(file) ? file : absolutePathPrefix + relative(destDir, file).replace(/\\/g, '/');
				if (onlinePath) { 
